...
def compute_dof_torque(cfg_ctrl, name, mode, only_move, p_gain, d_gain, f_gain, dof_pos, dof_vel,
                       pos, quat, linvel, angvel,                    
                       jacobian, mass_matrix,                       
                       ctrl_target_pos, ctrl_target_quat, ctrl_hand_target,
                       cur_target_contact_force, cur_contact_force,
                       ext_force_global, ext_torque_global, hand_type,
                       device):
    ...
    cur_wrench = torch.zeros((cfg_ctrl['num_envs'], 1), device=device)    
    dof_torque = torch.zeros((cfg_ctrl['num_envs'], 7), device=device)        
    if not (hand_type == "None"):
        hand_torque = torch.zeros_like(ctrl_hand_target)
        dof_torque = torch.cat((dof_torque, hand_torque), dim=-1)
    
    if cfg_ctrl['gain_space'] == 'joint':
        pos_error, axis_angle_error = get_pose_error(
            pos=pos,
            quat=quat,
            ctrl_target_pos=ctrl_target_pos,
            ctrl_target_quat=ctrl_target_quat,
        ...
        delta_arm_dof_pos = _get_delta_dof_pos(delta_pose=delta_fingertip_pose,
                                               ...
                                               jacobian=jacobian,
                                               device=device)
        dof_torque[:, 0:7] = cfg_ctrl['gym_default']['joint_prop_gains'] * delta_arm_dof_pos + \
                             cfg_ctrl['gym_default']['joint_deriv_gains'] * (0.0 - dof_vel[:, 0:7])

        if cfg_ctrl['do_inertial_comp']:
            # Set tau = M * tau, where M is the joint-space mass matrix
            arm_mass_matrix_joint = mass_matrix # need to slice for the link num (arm + hand)
        ...
        if cfg_ctrl['do_motion_ctrl']:
            ...
            if only_move:
                move_gain_ratio = 3.0               
            task_prop_gains_  = p_gain * move_gain_ratio
            task_deriv_gains_ = 2.0*torch.sqrt(p_gain) if critical_damping else d_gain
                        
            task_wrench_motion = _apply_task_space_gains(name=name,
                                                         mode=mode,
                                                         delta_pose=delta_pose,
                                                         linvel=linvel,
                                                         angvel=angvel,
                                                         task_prop_gains=task_prop_gains_,
                                                         task_deriv_gains=task_deriv_gains_,
                                                         ext_force_global=ext_force_global,
                                                         ext_torque_global=ext_torque_global)

            if cfg_ctrl['do_inertial_comp']:
                # Set tau = Lambda * tau, where Lambda is the task-space mass matrix
                jacobian_T = torch.transpose(jacobian, dim0=1, dim1=2)
                arm_mass_matrix_task = torch.inverse(jacobian @ torch.inverse(mass_matrix) @ jacobian_T)  # ETH eq. 3.86; geometric Jacobian is assumed
                task_wrench_motion = (arm_mass_matrix_task @ task_wrench_motion.unsqueeze(-1)).squeeze(-1)
                
            task_wrench = task_wrench + torch.tensor(cfg_ctrl[cfg_ctrl["ctrl_type"]]['motion_ctrl_axes'], device=device).unsqueeze(0) * task_wrench_motion            
        ...
        if cfg_ctrl['do_force_ctrl']:            
            ...
            task_wrench_force = task_wrench_force + cur_target_contact_force  # open-loop force control (building towards ETH eq. 3.96-3.98)            

            cur_contact_force = torch.clamp(cur_contact_force, max=20)
            if cfg_ctrl['force_ctrl_method'] == 'closed':
                force_error, torque_error = _get_wrench_error(
                    contact_force=cur_contact_force,                    
                    target_contact_force=cur_target_contact_force,
                    num_envs=cfg_ctrl['num_envs'],
                    device=device)
                
                # Set tau = tau + k_p * contact_wrench_error                
                task_wrench_force = task_wrench_force + f_gain * torch.cat((force_error, torque_error), dim=1)  # part of Modern Robotics eq. 11.61
                # task_wrench_force = task_wrench_force + f_gain # f_gain itself is the delta force
                
                cur_wrench = task_wrench_force[:,2]
       
            task_wrench = task_wrench + torch.tensor(cfg_ctrl['force_ctrl_axes'], device=device).unsqueeze(0) * task_wrench_force
            
        # Set tau = J^T * tau, i.e., map tau into joint space as desired
        # Set tau = J^T * F, i.e., map tau into joint space as desired        
        jacobian_T = torch.transpose(jacobian, dim0=1, dim1=2)                        
        dof_torque[:, 0:7] = (jacobian_T @ task_wrench.unsqueeze(-1)).squeeze(-1)     

        ctrl_type = cfg_ctrl["ctrl_type"] # task space impedance
        if hand_type == "Gripper":
            dof_torque[:, 7:9]  = cfg_ctrl[ctrl_type]['gripper']['joint_prop_gains']     * (ctrl_hand_target - dof_pos[:, 7:9])  + cfg_ctrl[ctrl_type]['gripper']['joint_deriv_gains']     * (0.0 - dof_vel[:, 7:9])   # gripper finger joints
        elif hand_type == "TheHand":
            dof_torque[:, 7:27] = cfg_ctrl[ctrl_type]['the_hand']['joint_prop_gains']    * (ctrl_hand_target - dof_pos[:, 7:27]) + cfg_ctrl[ctrl_type]['the_hand']['joint_deriv_gains']    * (0.0 - dof_vel[:, 7:27])  # TheHand finger joints        
        elif hand_type == "ShadowHand":
            dof_torque[:, 7:31] = cfg_ctrl[ctrl_type]['shadow_hand']['joint_prop_gains'] * (ctrl_hand_target - dof_pos[:, 7:31]) + cfg_ctrl[ctrl_type]['shadow_hand']['joint_deriv_gains'] * (0.0 - dof_vel[:, 7:31])  # TheHand finger joints        

    return dof_torque, cur_wrench

def get_pose_error(pos,
                   quat,
                   ctrl_target_pos,
                   ctrl_target_quat,
                   jacobian_type,
    ...
    if jacobian_type == 'geometric':  # See example 2.9.8; note use of J_g and transformation between rotation vectors
        ...
        quat_norm  = torch_utils.quat_mul(quat, torch_utils.quat_conjugate(quat))[:, 3]  # scalar component
        quat_inv   = torch_utils.quat_conjugate(quat) / quat_norm.unsqueeze(-1)
        quat_error = torch_utils.quat_mul(ctrl_target_quat, quat_inv)        
        ...
        axis_angle_error = axis_angle_from_quat(ctrl_target_quat) - axis_angle_from_quat(quat)
...
def _get_wrench_error(contact_force,                      
                      target_contact_force,
                      num_envs,
                      device):
    """Compute task-space error between target Franka fingertip contact wrench and current wrench."""

    contact_force = torch.zeros((num_envs, 6), device=device)
    contact_force[:,0:3] = contact_force[:,0:3]
    # Cols 3 to 6 are all zeros, as we do not have enough information

    force_error = (contact_force[:, 0:3]) - (-target_contact_force[:, 0:3])
    torque_error = target_contact_force[:, 0:3] - (target_contact_force[:, 0:3])
...
def _apply_task_space_gains(name,
                            mode,
                            delta_pose,
                            linvel,
                            angvel,
    ...
    lin_error = delta_pose[:, 0:3]        
    if mode == 'normal':
        task_wrench[:, 0:3] = task_prop_gains[:, 0:3] * lin_error + task_deriv_gains[:, 0:3] * (0.0 - linvel)
    elif mode == 'ext_force_comp':
        task_wrench[:, 0:3] = task_prop_gains[:, 0:3] * lin_error + task_deriv_gains[:, 0:3] * (0.0 - linvel) + ext_force_global * -1

    # Apply gains to rot error components
    rot_error = delta_pose[:, 3:6] 
    
    if mode == 'normal':
        task_wrench[:, 3:6] = task_prop_gains[:, 3:6] * rot_error + task_deriv_gains[:, 3:6] * (0.0 - angvel)
    elif mode == 'ext_force_comp':        
        task_wrench[:, 3:6] = task_prop_gains[:, 3:6] * rot_error + task_deriv_gains[:, 3:6] * (0.0 - angvel) + ext_torque_global * -1

    return task_wrench

def get_analytic_jacobian(quat, jacobian, num_envs, device):
    ...
    # Eq. 2.12 gives an expression for E_p_invE_r_inv
    ...
    axis_angle_cross = get_skew_symm_matrix(axis_angle, device=device)
    angle = torch.linalg.vector_norm(axis_angle, dim=1)
    factor_1 = 1 / (angle ** 2)
    factor_2 = 1 - angle * 0.5 * torch.sin(angle) / (1 - torch.cos(angle))
    factor_3 = factor_1 * factor_2
    E_r_inv = I \
              - 1 * 0.5 * axis_angle_cross \
              + (axis_angle_cross @ axis_angle_cross) * factor_3.unsqueeze(-1).repeat((1, 3 * 3)).reshape((batch, 3, 3))
...
def get_skew_symm_matrix(vec, device):
    ...
    batch = vec.shape[0]
    I = torch.eye(3, device=device)
    skew_symm = torch.transpose(torch.cross(vec.repeat((1, 3)).reshape((batch * 3, 3)), I.repeat((batch, 1))).reshape(batch, 3, 3), dim0=1, dim1=2)
